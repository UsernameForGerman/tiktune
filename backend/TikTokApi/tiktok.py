...
class TikTokApi:
    ...
    def getData(self, api_url, signature, userAgent, language='en', proxy=None):
        ...
        # print(url)
        """
        r = session.get(url, headers={"method": "GET",
        ...
        r = session.get(
            ...
            # proxies={
            #     'https': 'https://62.118.131.240:3128'
            # }
        )
        try:
            session.close()
    ...
    def getTikTokById(self, id, language='en', proxy=None):
        ...
        api_url = "https://tiktok.com/api/item/detail/?itemId={}&language={}&verifyFp=".format(
            id, language)
        # b = browser(api_url, proxy=proxy)
        # return self.getData(api_url, b.signature, b.userAgent, proxy=proxy)
        return self.getData(api_url, None, None)
